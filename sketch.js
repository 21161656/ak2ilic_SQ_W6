...
function preload() {
  ...
  paperImage = loadImage("assets/images/paper.webp");
  denyStampImage = loadImage("assets/images/denyStampImage.png");
  ezicImage = loadImage("assets/images/ezic.png");
  dimitriImage = loadImage("assets/images/dimitri.png");

  music = loadSound("assets/sounds/papersplease.wav");
...
function drawBackground() {
  ...
  // Main Papers Please background
  image(bgImage, 0, 0, WORLD_W, WORLD_H);

  // Scattered paper images instead of random squares/circles
  for (let i = 0; i < bgShapes.length; i++) {
    let s = bgShapes[i];

    // Skip papers far from the camera view
    if (
      s.x < camX - s.size ||
...
else if (playerRow === 1) { // up
  offsetX = OFFSET_UP.x;
  offsetY = OFFSET_UP.y;
}
else if (playerRow === 2) { // right
  offsetX = OFFSET_LEFT.x;
  offsetY = OFFSET_RIGHT.y;
}
else if (playerRow === 3) { // left
  offsetX = OFFSET_LEFT.x;
  offsetY = OFFSET_LEFT.y;
}
...
function drawWinScreen() {
  ...
  textAlign(CENTER);
  textSize(52);
  text("Boss Dimitri Defeated!", width / 2, height / 2 - 30);
